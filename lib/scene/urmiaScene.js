 ...
 *
 * حالت «overlay» برای وقتی است که ویدئوی واقعی پل میانگذر پشت صحنه پخش
 * می‌شود: آن‌جا دریاچه، پل و کوه‌های مدل‌شده حذف می‌شوند تا روی نسخه واقعی
 * خودشان نیفتند و فقط لایه فنی — چرخ‌دنده، بلور نمک و ستاره‌ها — می‌ماند.
 *
 * @param {typeof import("three")} THREE
 * @param {HTMLElement} el ظرفی که بوم رندر داخلش می‌نشیند
 * @param {{ onReady?: () => void, layers?: "full" | "overlay" }} [options]
 * @returns {{ dispose: () => void } | null} اگر WebGL در دسترس نباشد null
 */
export function createUrmiaScene(THREE, el, options = {}) {
  const overlay = options.layers === "overlay";
  let renderer;
  try {
  ...
  const FOG = 0.021;
  if (!overlay) scene.fog = new THREE.FogExp2(night.getHex(), FOG);
  ...
  if (!overlay) {
    scene.add(deck);
    piers.forEach((pier) => scene.add(pier));
  }
  ...
  if (!overlay) scene.add(flamingos);
  ...
  const tick = () => {
    ...
    if (!overlay) lakeMat.uniforms.uTime.value = t;
  ...
  return {
    dispose() {
      ...
      [lake, deck, ...piers, flamingos, ridge].forEach((obj) => {
