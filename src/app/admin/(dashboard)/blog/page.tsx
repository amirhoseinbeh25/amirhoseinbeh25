...
export const metadata = { title: "بلاگ | Kemkan Admin" };
  ...
  return (
    <div>
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-extrabold text-ink">مقالات بلاگ</h1>
        <Link
          href="/admin/blog/new"
      ...
      <div className="mt-6 rounded-2xl border border-paper-soft bg-paper divide-y divide-paper-soft overflow-hidden">
        {posts.map((p) => (
          <div key={p.id} className="flex items-center justify-between gap-4 px-5 py-4">
            <div className="min-w-0">
              ...
              <p className="text-xs text-ink-soft truncate">{p.date} · {p.slug}</p>
            </div>
        ...
        {posts.length === 0 && <p className="px-5 py-8 text-center text-ink-soft">هنوز مقاله‌ای ثبت نشده است.</p>}
      </div>
    </div>
  );
}
